...
<!-- v10 review 阶段全文核验修正 + WebSearch 补充搜索的新发现, 均为背景资料, 不构成本 topic 核心因果机制的撞车. -->

- **勘误 (全文核验, 非新论文): Flora, Varga, Potvin, Lang (2026), arXiv:2603.20250 的 U-Net 输入结构**。通过 arxiv-tools 下载并读取 `b-methods.tex` 全文核实: 该 U-Net 的输入并非原始 pressure-level x longitude x latitude x time 4D 张量, 而是先把 3km WoFS 集合成员场粗化到 9km 网格、按 intrastorm/environmental 变量分别做时间维度的 max/mean 聚合、再计算集合统计量后拼接成的 63-channel 2D (无独立时间轴) 张量 (对照: 同一数据流程给表格模型产出 175 个聚合特征, 见原文 "After the data processing, the tabular dataset has 175 inputs, while the deep learning dataset has 63")。即该 U-Net 确实端到端训练在网格化输入上, 但其输入本身已经过时间聚合与空间粗化, 不是保留完整原始时间序列的 4D raw tensor。此前 landscape 中对该文的既有描述 (见上方同日期条目) 称其为"在原始格点场上端到端训练", 该表述对"格点"成立、对"原始时间序列"不成立, 特此勘误备查 (原条目按 append-only 规则不删改, 仅在此追加澄清)。
- **Liu, Huang, Lall (2026), "Instability-Aware Steering of an Extreme Atmospheric River in an AI Weather Foundation Model"** (arXiv:2604.18906; abstract-verified via arxiv-tools info)。用 Aurora AI 天气基础模型, 结合有限时间 Lyapunov 指数与 jet-eddy 相互作用判据识别对下游演化敏感的上游位置, 对一次加州极端大气河 (AR) case study 施加理想化的"云播种"扰动, 评估是否能引导 AR 强度/落地位置改变。撞车无: 此处"instability"指动力系统的有限时间 Lyapunov 指数敏感性/混沌可控性, 目的是主动 steering/intervention, 与本 topic "因果效应估计本身的不稳定性作为被动预警指标"是完全不同的机制与目标, 仅背景层面提示"instability-aware"一词在 AI 天气建模文献中也被用于动力系统敏感性分析这一独立含义。
