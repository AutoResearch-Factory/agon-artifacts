...
## 新增相关论文/数据产品 [idea-reviewer, 2026-07-16]

- **MYRORSS Storm Cluster Dataset for Tornado Detection and Climatology (2005-2011)** (Zenodo, DOI:10.5281/zenodo.19644586, v1 发布 2026-04-18; Wenjun Cui, Kiel Ortega, Skylar Williams; CC-BY 4.0; 43.7 GB) [核验方式: WebFetch Zenodo 记录页原文 (一手来源), 无 arXiv/期刊伴随论文被索引到, AMS 检索未命中同作者组合 2025-2026 论文]: 从 MYRORSS 雷达反射率识别的 CONUS 风暴对象簇 (2006-2011), 每簇附 NEXRAD WSR-88D 雷达参数 + RUC (Rapid Update Cycle) 分析提取的环境特征, 经 NOAA Storm Events 共位标注 tornado/hail/wind 标签, 并发布 ML-ready 特征集、预处理训练/评估数据与两个训练好的模型 (CatBoost/Random Forest, 龙卷检测与强度分类)。**与本 topic 的关系**: 这是对 v2 review 所指 "MYRORSS 缺现代 ML 基准封装" 空缺的直接占位——"多灾害标签 + 雷达 + 近风暴环境特征 + ML-ready + baselines + DOI/版本化开放发布" 这一宽松组合层面主张自 2026-04 起不再空缺。但该产品是**对象级标量特征表** (簇级提取特征), 时代为 2006-2011 RUC (非 HRRR/MRMS 现代档案), 无原始时空场序列、无长时窗环境演化/正常态对比、无 issue/valid/availability-time 语义、无 nowcasting 序列任务。本 topic 剩余可辩护差异轴: 原始演化场事件包、现代 HRRR/MRMS 档案、issue-time 可审计语义、环境演化+正常态对比一等用途、泄漏审计的概率/nowcasting 评测协议; idea 的 novelty 表述须显式引用并对表该产品。
- **UniExtreme** (arXiv:2508.01426, v2 2025-09-04; Hang Ni, Weijia Zhang, Hao Liu; 投 KDD 2026) [核验方式: WebFetch arXiv abs 页 (S2/arXiv API 均 429 限流, 未读全文)]: 极端天气预报基础模型 (Adaptive Frequency Modulation + Event Prior Augmentation), 实验建立在 HR-Extreme (HRRR 衍生) 等既有数据集上; 方法论文非数据产品, 不构成撞车, 仅作为 "极端天气 FM 线持续消费 HRRR 衍生数据集" 的旁证。

## 对照数据集: 交付物与目标差异审计 [dispatcher-commissioned audit, 2026-07-16]

**本数据集 (scs-env-benchmark) 参照基准**: 风暴中心事件包 = 长时窗 HRRR 环境场**原始序列** (设计值 -15d/+5d, 覆盖事件前正常态时段; 窗长为 refine 中可调参数) + MRMS 雷达序列 + NOAA Storm Events 实况标签 + IEM/NCEI 预警记录 (预警与实况分离交付, issue-time 可审计); 三个一等用途: 环境演化/正常态对比分析、概率化事件预报、雷达锚定环境约束 nowcasting。下表逐一对表两轴: **结果差异** (它实际交付什么 vs 我们交付什么, 双向诚实) 与 **目标差异** (它为何而建 vs 我们为何而建)。

| 数据集 | 数据产品实际交付物 | 建设目标 | 与本数据集的结果差异 | 与本数据集的目标差异 | 证据来源 |
|---|---|---|---|---|---|
| **MYRORSS Storm Cluster Dataset 2026** (Zenodo DOI:10.5281/zenodo.19644586, 2026-04) | 簇级**特征表**: ~1060 列 / ~92 变量族, 每列是按雷达探测风暴对象计算的汇总统计量 (mean/max/std/百分位/bin 计数); 变量族混合雷达派生量 (回波顶 ET18/40/50、层厚、闪电密度) 与 RUC 分析派生 NSE 标量指数 (MUCAPE、ML100mbAvgCAPE、DCAPE 变体、LLShear/MLShear/0-6km 平均切变、直减率、LCL 高度、RH 层、theta-e/冻结层高度、层位风)。文件: ClusterData.zip 43.6GB + 变量清单 docx + CatBoost/RF 两个训练模型 zip。Storm Events 共位 tornado/hail/wind 标签; CC-BY 4.0, DOI 版本化。时代: 2006-2011, RUC 13/20km 逐时 (非 HRRR 3km) | 龙卷检测与气候学的对象级 ML-ready 特征集 + 现成基线模型 | **关键区分 (须精确表述): 它交付的是"附着在雷达对象上的派生环境参数统计量", 不是场意义上的环境数据** — 无格点/原始环境场、无垂直廓线、无环境时间序列、无事件前窗口、无正常态时段。我们交付原始演化场序列。反向: 它已有开箱即用的特征表 + 两个 baseline 模型 + 多灾害标签 + DOI 版本化, "ML-ready 组合层面"占位成立; 我们的标量特征层需另行派生 | 它为"雷达对象 → 灾害分类"监督学习与气候学统计服务, 环境仅是特征来源; 我们把环境本身当研究对象 (演化分析/正常态对比/issue-time 审计) | Zenodo 记录页 + `storm_cluster_variable_list.docx` 实际下载核验 (dispatcher, 2026-07-16)。注: 此前 review 中 "RUC environment features" 的宽泛说法以本条为准修正 |
| **MYRORSS 原始** (BAMS 103(3), 2022) | 1998-2011 CONUS 多雷达再分析格点档案: 0.01°、~5min、3D 反射率 + 派生产品 (回波顶、MESH、VIL/SHI、两种方位切变层产品), 垂直 250m-1km 间距; 处理侧用 NSE 模式分析, 但 AWS 开放档案交付物是雷达派生格点产品; NODD 开放, 静态不更新, 无 ML 封装 | 长年代一致的多雷达融合档案, 供气候学/算法开发 (产品档案, 非 benchmark) | 无事件封装、无标签/split/baseline、无 HRRR 级环境场序列; 我们是事件包 + 标签 + 预警 + 环境序列。反向: 它 14 年历史深度与 CONUS 全覆盖是我们事件包形态不可替代的 | 档案分发 vs 三用途研究基准; 它是"雷达+近风暴环境捆绑发布"先例 (novelty 措辞须组合层面化) | AWS Open Data registry 页 WebFetch (2026-07-16) + landscape 2026-07-13 核验条目 |
| **TorNet** (2401.16437, AIES 2025) | 203,133 个 netCDF 极坐标雷达 chip (80km×60°×2 仰角×4 帧≈20min, 6 个 Level-II 变量), 2013-2022; confirmed/警报负样本/random 三类采样; 帧级二元龙卷标签; 防泄漏 split; TVS/LR/RF/CNN 基线, 代码+权重开源 | 龙卷检测/短时预测 ML benchmark | 零环境场、单 hazard、~20min chip 快照式; 我们多灾害 + 环境序列 + 长窗事件包。反向: 它有全分辨率偏振 Level-II 原生极坐标 (我们 MRMS 层不保留单站径向速度) 与成熟的困难负样本/防泄漏设计 | 检测器 benchmark vs 环境-事件研究数据产品; 其 future work (NWP/卫星/闪电融合) 恰是我们主体, 11 篇反引文无一做模态扩展 — gap 书面证据 | wiki/2401.16437.md (全文核验) |
| **HR-Extreme** (2409.18885) | 2020 全年 CONUS 17 类极端事件 HRRR 3km 裁剪包: (n, t=3h, 69 变量, 320², mask), npz, 5TiB; Storm Events+SPC 报告 DBSCAN 聚类定义事件; Pangu/FuXi/NWP 评测 + HR-Heim 基线; 生成接口可任意年份/前后小时 (但交付形态是 3h) | 高分辨率极端天气**预报误差评测集** (暴露 DL 极端退化 +34%~394%) | 交付形态为 t=3h 事件同期快照、69 个基本状态量、无对流派生参数、无雷达、无 provenance/QC/DOI; 我们: 长窗环境演化 + MRMS + 预警 + ESSD 级整编。**注意**: 它论文内做过"事件 vs ±15 天内随机正常时刻"统计对比, 但正常态时段只是论文内验证分析, **不在交付数据里**。反向: 其 DBSCAN 聚类管线/Herbie/打包策略已开源可复用 | 评测既有预报模型的误差 vs 支撑环境演化分析/预报/nowcasting 三用途; 其 future work 自点名 "precursors" 缺口 | wiki/2409.18885.md (全文核验) |
| **Storm250-L2** (2510.16031, **已撤稿**) | (声明) GR-S track 上 250m 单站 NEXRAD Level-II 极坐标反射率事件包 (HDF5 + SHA-256 manifest); 实际数据从未上线; **2026-06-25 撤稿, 理由"数据集构建产生的事件覆盖不足以支撑预期 ML 用途"** | 亚公里分辨率 nowcasting 数据集 | 纯雷达单模态、reflectivity-only、零环境场、零标签任务化, 且以失败告终 (无实际交付物) | 攻分辨率轴 vs 我们攻模态/时窗轴; 其撤稿是本 idea 风险登记中"事件覆盖率"风险的实证前车之鉴 | wiki/2510.16031.md + landscape 撤稿记录 (2026-07-13) |
| **SEVIR** (NeurIPS 2020) | ~1 万+ 事件 × 5 模态时空对齐影像 (GOES-16 C02/C09/C13 + NEXRAD VIL 拼图 + GLM 闪电), 384×384 km、固定 4h 窗、5min 级 cadence, HDF5, AWS Open Data "无使用限制", 部分事件链接 Storm Events; 每年增补 | 多模态风暴影像 nowcasting/生成 benchmark | **不含任何 NWP/环境场**; 固定 4h 事件窗 (无事件前环境、无正常态); VIL 影像化产品而非 3D 反射率; 无预警记录。反向: 卫星 + 闪电两模态是我们 v1 没有的; 其五模态对齐工程与逐年增补机制成熟 | 影像序列预测 benchmark vs 环境-事件研究数据产品 | landscape 2026-07-13 NeurIPS 页核验条目 + AWS registry WebFetch (2026-07-16) |
| **GridRad-Severe** (Murphy et al. 2023, NCAR GDEX d841006) | 2010-2023 每年 ~100 个最强事件日的事件域 4D 雷达体扫: 0.021°、5min、7 变量 (含偏振/AzShear/径向散度; 2010-12 无偏振), 28 垂直层, 含风暴 track; DOI 10.5065/2B46-1A97, 不规则更新 | 强对流事件日雷达研究档案 (过程研究/风暴目录构建) | 无环境场交付 (2503.15466 的 GR-S×HRRR 配对是论文内部产物、未发布; SFCOA 需向 SPC 申请)、无 ML split/loader/baseline (2605.24067 手搓 "3D-NEXRDA" 即证)、事件日制而非确认事件包、无标签/预警封装。反向: 它有真 3D 偏振体扫与 28 层垂直结构, 优于我们 MRMS 2D/2.5D 产品层 | 科研档案 vs benchmark 化数据产品; 它是我们雷达锚定/事件 scaffold 的上游候选而非竞品 | GDEX d841006 页 WebFetch (2026-07-16) + wiki/2503.15466.md + wiki/2605.24067.md |
| **FunnelCloud** (2017, DOI:10.1080/17538947.2017.1279235) | (摘要级) 云端探索系统: 对龙卷报告记录做 ST-DBSCAN 时空聚类, 簇关联 NARR 再分析环境场 (解码为图像存 GridFS) 与 NEXRAD 反射率 (转 JSON/BSON 入库), 供时空统计/数据挖掘查询 | 龙卷 outbreak 与气候指标关系的交互探索系统 | 交付的是查询系统/数据库而非可再分发数据产品; 环境场以 NARR **图像**形态存储 (非分析就绪场序列); 单 hazard; 无 splits/baselines/DOI 版本化/issue-time 语义。反向: "报告聚类 + 环境 + 雷达关联"三件套组合 2017 已在先, novelty 措辞须显式引用并划界 | 探索查询系统 vs 训练/分析就绪的开放基准 | **abstract-only** (全文两独立源 403; landscape 2026-07-15 多源交叉核验条目) |
| **MeteorPred / MP-Bench** (2508.06859) | 421,363 对 ERA5 4D 场 (0.25°、185 通道、预警发布后 [t, t+11] 12h 窗) × CMA 预警文本; 四类 QA 任务; MLLM 基线 + 消融; 年际切分; 代码数据投稿时未释出 | MLLM 灾害天气预警文本理解/生成 benchmark | 环境是 0.25° ERA5 (非风暴尺度)、窗口锚定预警发布**后** (零提前量, 实为事后诊断)、标签是预警文本而非实况、无雷达、无 SCS 专用类别; 我们: HRRR 3km 事件前长窗 + 实况标签, 预警单独作为 issue-time 审计层而非真值。反向: 它 42 万对规模与 MLLM 评测协议成熟 | 评测层 (测 MLLM 能力) vs 数据底座层; 最近邻 prior work 但处于不同 stratum | wiki/2508.06859.md (全文核验) |
| **EWB** (2605.01126) | 2020-2024 五类高影响事件案例库 + 观测靶 (站点/报告/IBTrACS/IVT) + 影响指标 + 自建多模型再预报档案 (Zarr+Icechunk); 代码 PyPI; 环境仅以 CBSS=MLCAPE×shear **单一标量**进入评测 | 全球 0.25° AI/物理模型的高影响天气**验证框架** | 无训练数据、无雷达、无对流尺度场、环境压缩成单标量、无时间演变; 我们是其下方缺失的数据层。反向: 其边缘/负样本日判据与影响指标体系可直接进我们评测协议; 其全球事件覆盖 (加/澳) 超出我们 CONUS 范围 | 评测协议 vs 数据产品; 互补大于竞争 (我们可为其提供对流尺度靶) | wiki/2605.01126.md (全文核验) |
| **WxC-Bench** (2412.02780) | 6 个独立下游任务数据集松散并置 (MERRA-2/ERA5/卫星记录/HURDAT/SPC 讨论文本; HF 托管 per-task loader), per-task 基线; 格式混杂, 无统一封装 | 天气气候基础模型 (Prithvi WxC 等) 下游微调/泛化评测 | 无风暴尺度事件包、无雷达、无 Storm Events 标签; 环境场是单时刻快照用法 (湍流任务仅 1800UTC); 无 provenance/license 讨论。反向: 任务谱系组织法与 HF per-task 托管模式可借鉴 | FM 泛化评测 vs 事件级 SCS 研究底座 | wiki/2412.02780.md (全文核验) |
| **MeteoNet** (Météo-France, 2020) | (brief) 2016-2018 法国两个 550km 域开放多源集: 雨雷达 (0.01°/5min)、卫星、地面站、AROME/ARPEGE NWP 输出、地形/海陆掩膜; 开放许可 | 面向数据科学社区的通用开放气象多源数据 (nowcasting 等) | 连续档案切片而非事件包; 无强对流事件标签/预警任务; NWP 场无事件对齐与 issue-time 审计语义。反向: 它是少数把 NWP 与雷达同盒发布的先例 | 通用开放数据 vs SCS 专用事件基准 | 公开常识级 (meteonet.umr-cnrm.fr), 按指令未深挖 |
| **HKO-7** (Shi et al., NeurIPS 2017) | (brief) 港天文台 2009-2015 雷达 CAPPI 反射率影像 (480×480, 512km 域, 6min), TrajGRU 配套 nowcasting benchmark (balanced MSE/MAE); 获取需与 HKO 签协议, 非完全开放再分发 | 降水 nowcasting 方法 benchmark | 雷达单模态、区域局部、无环境场/标签/事件封装、许可受限 | 方法评测经典基准 vs 开放多模态数据产品 | 公开常识级, 按指令未深挖 |
| **CLLMate** (2409.19058) | 7,747 条新闻抽取事件 × 事件**当日** ERA5 日均 4 变量 RGB 缩略图 + 统计量; 多选题评测; 23 个 MLLM 结果; 数据审后释出承诺 | MLLM 天气气候事件"预报" (实为同日诊断) 评测 | 单日快照、RGB 三通道压缩、新闻标签 (媒体覆盖偏差)、无雷达/官方事件库/时间窗; 数据形态完全不同。其 RGB 压缩失败消融反向支持我们"交付原生栅格"设计 | 评测层 MLLM 基准 (不同 stratum) vs 数据底座 | wiki/2409.19058.md (全文核验) |
| **WeatherQA** (2406.11217, NeurIPS D&B 2024) | 8,511 条 SPC MCD 文本 × 20 张**预渲染** CONUS 中尺度分析图 (发布时刻单快照), 2014-2020 (渲染档案断供致止于 2020); MCQ/分类任务 + 4 位 SPC 预报员评分; CC0 | VLM 强对流"配料法"推理评测 | 预渲染图而非原始场、单时刻快照 (作者自认缺时间演变)、标签是预报员产品无实况校验、数据不可延展; 我们: 原始场序列 + 实况标签 + 不依赖渲染产品存废。反向: 其 20 配料参数清单是我们 HRRR 变量选型起点 | 评测层 VLM 基准 (不同 stratum) vs 数据底座 | wiki/2406.11217.md (全文核验) |

**综合判定**。(1) **全表唯一轴** (15 行无一具备): ① *原始环境场的事件对齐长时窗序列* — 含环境信息的行要么是标量特征统计 (MYRORSS-cluster、EWB 的 CBSS、FunnelCloud 的图像化 NARR), 要么是短窗/单时刻快照 (HR-Extreme 3h、WxC-Bench 单时刻、MeteorPred 12h 事后窗、CLLMate 单日、WeatherQA 单时刻), 要么完全没有 (TorNet/SEVIR/Storm250-L2/GridRad-Severe/HKO-7/MYRORSS 原始档案交付层); ② *覆盖事件前正常态时段的窗口* — 唯一逼近者 HR-Extreme 只在论文内部用 ±15d 随机正常时刻做过统计验证, 正常态数据不在任何一家的交付物中 (我们把它升格为交付物属性时应引用该分析); ③ *issue-time 可审计语义 (预警记录与实况标签分离交付)* — 全表无一提供, MeteorPred 把预警当真值恰是反例。(2) **与至少一个近邻共享的轴** (novelty 表述不能单独依赖): 多灾害确认标签 (HR-Extreme、MYRORSS-cluster、SEVIR 部分链接)、风暴中心事件封装 (TorNet/SEVIR/Storm250-L2/GridRad-Severe)、现代 HRRR 时代 (HR-Extreme)、MRMS 级融合雷达 (MYRORSS/SEVIR)、"ML-ready + baseline + DOI 版本化"组合 (TorNet、MYRORSS-cluster、SEVIR)。(3) **最近邻排序**: HR-Extreme (骨架逐步同构, 差在交付形态/模态/整编标准) > MYRORSS Storm Cluster 2026 (宽松组合主张的占位者, 但"对象级参数统计 vs 场数据"的种类鸿沟清晰) > GridRad-Severe (雷达底座 + 事件日制, 环境配对存在于文献但从未发布)。(4) **意外发现**: (i) HR-Extreme 的 ±15d 正常态对比先例 (论文内验证, 非交付物) 此前未被任何 review 注意; (ii) MYRORSS-cluster 经变量清单实际核验后, 其威胁从 v3 review 时的"组合占位"降级为"不同数据种类"— 我们的差异不是"更新的时代"而是"特征统计 vs 场数据"这一根本区别, 时代差 (RUC vs HRRR) 只是叠加项; (iii) 没有任何一行比先前认知**更近**: 审计后无新增撞车, FunnelCloud (摘要级) 仍是聚类选型层面唯一需显式划界的在先系统。
