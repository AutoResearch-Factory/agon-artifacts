...
<deep-lit-integration date="2026-07-11" scope="idea" iteration="3">

<!-- 本次为 idea-scope deep-lit-tick 第3次运行 (第1次26篇+第2次18篇, 已读基线138篇, 已加载 already_read_ids 去重, 与 topic-scope 共用同一 wiki 池)。本轮精读51篇 (7轮: 8+8+8+8+7+6+6), 用户特别要求聚焦 v3 pilot 核心新发现的两个具体问题: (a) 能否设计更干净的受控对比隔离 Delta = AUC_conditional - AUC_marginal, 回应 v3 review 指出的"双 regime 混淆比较"批评; (b) 是否已有"因果条件化丢弃预测信号"的理论解释。完整51篇清单及分组见 `ideas/causal-scs-indicator-deep-lit-report-2026-07-11-iteration3.md`。核心结论: 0 篇构成 novelty 撞车 (v3 novelty quick-check 现有最强撞车对比对象排名不变); 但意外收获了两条独立的高价值理论支柱, 直接回应 v3 reviewer 的核心质疑。 -->

- **支柱一: "因果条件化丢弃预测信号"已收敛为 7 种独立技术路线的一致结论 (建议写入 Narrative/Strongest objection 段)**——这不是本课题臆造的现象, 而是横截面/多环境因果不变性文献里反复独立发现的结构性规律, 只是从未在滑动时间窗口+趋近分岔场景下被检验过:
  1. **CI 检验功效随条件集增大指数/多项式下降** (Yu et al. 综述 arXiv:1911.07147、Yu-Liu-Li arXiv:1802.05844、Peters-Bühlmann-Meinshausen ICP 原始论文 arXiv:1501.01332)。1802.05844 证明因果 FS 与非因果 FS 本是同一目标函数 (Markov Blanket 互信息最大化) 的不同求解路径, 因果法条件集阶数随变量数指数增长, 小样本高维时非因果法几乎全面胜出。1501.01332 给出该失效模式最早的数学证明: 检验矩与扰动矩错配 (如信号仅体现在二阶矩) 时弱化零检验会完全丢失应识别的因果变量。
  2. **Anchor regression minimax 决策论边界**: 扰动作用位置决定支配方向 (Rothenhäusler-Meinshausen-Bühlmann arXiv:1801.06229 证明扰动作用于 Y/隐混淆 H 而非协变量 X 时, 非因果方法一致地支配因果参数, 即使因果参数完全正确)。此边界被 Kostin-Jalaldoust-Bareinboim (arXiv:2606.12680) 用 Le Cam 两点法升级为**匹配的有限样本 minimax 上下界** (margin Δ≲1/n 时无算法能获益, 信息论硬下界而非仅经验现象), 并被 Kostin-Gnecco-Yang (arXiv:2502.02710) 推广到"测试扰动含未见方向"场景: 全部扰动落在未见方向时, 因果不变性方法与 OLS 风险增长率完全相同 (优势归零), K562 真实基因数据验证。**此边界条件与我们的双 regime 结构高度同构**: fragility regime (无真实图变化) ≈ 扰动全部落在"未见方向"(没有可利用的已知图变化方向); 正对照 regime (已知图变化) ≈ 扰动落在已张成/已知方向。
  3. **IRM 在环境数 ≤ 环境特征维度时全局最优解被非因果解支配, 且是"近乎默认状态"而非边缘情形** (Rosenfeld-Ravikumar-Risteski, ICLR2021, arXiv:2010.05761; 被 Kamath et al. AISTATS2021 arXiv:2101.01134 进一步证明 IRM 的线性/标量松弛类别本身造成与环境数无关的系统偏误)。
  4. **检验矩与扰动矩错配** (同上 1501.01332, 及非线性 ICP arXiv:1706.08576 给出 3 个具体数学反例)。
  5. **计算-统计缺口/平均情形困难** (arXiv:2606.12930, ICML2026: 某些不变子空间统计上可识别但多项式时间恢复困难, 类比 Planted Clique, 给出显式相变阈值)。
  6. **反例支柱 (证明是 regime-dependent 而非单向)**: 环境异质性充分时因果方法反而碾压非因果方法 (Pfister-Bauer-Peters CausalKinetiX, arXiv:1810.11776, 一致性定理精确刻画"因果何时赢"取决于唯一性条件 C3)。
  7. **参数计数不对称** (Bengio et al. 元迁移论文, arXiv:1901.10912): 扰动作用于真实原因变量时正确因果分解仅需重估计 O(N) 个参数, 错误分解因贝叶斯定理牵连需 O(N²), 与其余 6 种机制数学工具完全独立却指向同一结论。此外该文附录发现一个新的正交 regime 边界轴——判别信号在"post-shift 样本量"上非单调 (约10-20样本处达峰值后随样本增多消失), 建议在 fragility regime 设计中检查是否存在类似非单调窗口效应。
  - **大规模实证审计独立佐证 fragility 现象非孤例**: Nastl & Hardt (arXiv:2402.09891, 16个真实表格数据集/超50万模型) 全特征预测器 16/16 Pareto 支配纯因果特征预测器, ICP/PC/FCI 真实数据产出率近零; DomainBed (arXiv:2007.01434, 近4.6万网络) 严格协议下 IRM 平均分反低于 ERM, 唯一决定性胜利建立在信息泄漏的 oracle 调参协议上。
  - **可直接迁移/改造的受控 factorial 设计模板 (回应"隔离 Delta"的具体操作化需求)**: Magliacane et al. (arXiv:1707.06422) 的 γ(扰动强度0.1→100)×N(样本量) 二维扫描 (γ=0.1时因果法无优势甚至更差, γ≥1单调转强); Bühlmann Neyman Lecture (arXiv:1812.08233) Table 1 的模型复杂度×扰动强度×learner 三因子设计 (相对增益 +83%→-100.8% 翻转); Rosenfeld et al. (arXiv:2010.05761) 附录的环境数/维度比×测试扰动方向(same/reversed) 双轴设计 (观测到清晰相变); round1 已读的 Zuo & Wang (arXiv:2601.02322) 三分支设计 (每次仅扰动隐藏父节点/观测父节点/代理变量之一)。这四个模板的共同结构是"仅改变一个因子, 固定其余", 可直接改造为仅扰动 {conditioning 是否启用 / phase 预事件-事后 / 图变化强度 / 混淆强度} 之一的下一版 factorial 设计。

- **支柱二: Local independence 文献空白, 高强度 novelty 证据 (round6-7 新支线, 由 2001.06208 触发)**——连续时间/点过程原生因果发现理论 (而非从横截面理论类比迁移) 系统性追索的结论: **local independence (而非条件独立) 是连续时间动力系统的正确统计概念, 但截至2026年该文献仍完全未处理非平稳/趋近分岔/参数漂移场景**。证据链: Didelez (arXiv:0710.5874, 2008, 该领域奠基论文) 本人在 Extensions 一节明确提出应放松"图结构对所有 t 恒定"这一假设, 称其"deserves further investigation"; 19年后, 该问题仍未被解决 —— Mogensen (arXiv:2310.13796, AISTATS2024) 公理化了 local independence 图的 faithfulness 但全文不讨论非平稳/趋近分岔; Didelez 本人参与的最新延伸 (arXiv:2202.02311, 2022) 附录提供唯一的正式 faithfulness 违反反例, 但是静态代数构造而非趋近临界值式的渐进坍缩; 2310.13796 与 2202.02311 互相点名对方为"下一步应读"目标, 本轮 B7 反向扩展验证了这一闭环, 三角确认该空白稳定存在。**这是比"检索多轮未找到"更强的 novelty 证据形式** (领域奠基者本人主动标记为开放问题且长期未解决), 建议写入下一版 proposal 的 novelty 论证部分。
  - **附带发现的竞争解释候选** (建议在 Strongest objection 段讨论): Mogensen (2310.13796) 数值实验显示, 同一 local independence/Granger 框架内, 用"更多更小条件集"策略 (PC 类, 与 PCMCI+ 增量邻接搜索同构) 比"单次大条件集"检验更易因 (近)faithfulness 违反出错——这与我们 v3 Pilot 中 causal 在正对照 regime 数值也略低于 non-causal (0.786 vs 0.849, 而非追平) 的观察吻合, 提示 PCMCI+ 判别力损失可能部分是其增量条件集策略自带的 regime-general penalty, 而非分岔专属现象, 需要设计消融排除。
  - **精确的结构性错配机制候选**: Christgau-Petersen-Hansen (arXiv:2203.13559) 给出 non-collapsibility 定理——对真实满足条件局部独立的完整系统的边缘化子系统做参数化 CI 检验, 会导致检验完全丧失渐近水平 (不只是功效下降)。若能建立"趋近分岔 → 快变量被有效边缘化"这条因果链 (需要我们自己论证, 论文本身未处理), non-collapsibility 是可直接引用的定理级机制。
  - **局限**: 2506.07844 (连续时间 Itô 过程 local independence 检验) 明确指出该框架的渐近理论要求样本量 N→∞ **与** 时间步长 δ→0 联合发生, 这与 PCMCI+ 离散滑动窗口协议的渐近假设不同构, 引用时需注意这是概念/直觉层面的类比支撑, 非可直接套用的定理。
