...
## Deep-lit 补充发现 (2026-07-11, idea-scope deep-lit-tick iteration 2)

<!-- 本节由 idea-scope deep-lit-tick 第 2 次运行追加, 不修改上方已定稿 review 的分数. 本轮精读 18 篇 (5 轮, B4 饱和: 第 5 轮起每轮反向扩展新增候选降至 1-2 篇且均落入已高度覆盖的同一作者簇 [Runge/Wahl/Faltenbacher/Machlanski/Butler/Gideon-Stein/Petersen], 判定饱和). 聚焦 v2 idea 四个具体设计点: 双族 D/J profile、synthetic stop gate、matched negative controls、PCMCI+/LKIF. 全部 18 篇均不构成新的 novelty 撞车 (逐篇撞车判断见对应 wiki "Read by" 章节). -->

### 回应"双族 D/J profile"设计

- **本轮最强撞击**: arXiv:2410.19412 (RCV-PCMCI/VarLiNGAM, Yu/Guo/Luk 2024) 与其 2026-02 更新版 arXiv:2602.21381 (VCDF) 的 Consistency(C)/Variability(V) 跨折统计量与本 idea 的 (D,J) 双通道**数学同源**, 但用途相反 (过滤不稳定性 vs 利用不稳定性)。**VCDF 论文本人在结论中承认**: 当因果结构真的随时间变化时, 跨折不一致可能反映真实变化而非噪声——但从未做实验验证, 这正是 idea 想要填补的具体空白, 可作为最有力的 novelty 佐证直接引用("连该框架的原作者都承认这个问题未解决")。
- arXiv:2502.14719 (Faltenbacher/Wahl/Herman/Runge, PCMCI+ 团队本人 2025-02) 提出 "coherency score": 复用 PC 系方法(含 PCMCI)已执行的 CI 检验日志, 零额外计算成本判断输出图与自身检验结果是否自洽, 并给出 "increasing subsets" 启发式区分统计噪声 vs 结构性假设违反。**可作候选第三诊断通道**, 待官方 PCMCI+ gate 跑通后可直接复用其 CI 检验日志。
- arXiv:2402.04952 (Wahl & Runge) 的 separation-based 图距离族 (parent-SD 等) 与 Tigramite 生态兼容, 可用于让 J 通道 (相邻窗口 DAG 变化) 的定义更严谨, 但作者本人未在时序/PCMCI 数据上验证。

### 回应"synthetic stop gate"设计

- arXiv:2605.03045 (TCD-Arena, Stein/Penzel/Piater/Denzler, ICLR 2026, R-Score S/22): 33 类假设违背 × 16 数据 regime × 10 因果发现方法 (含 PCMCI/PCMCI+) 的大规模鲁棒性基准 (约 3600 万次因果发现尝试)。**可直接作为 Stage-1 synthetic gate 的现成正对照 regime 库** (V_stat/V_coef 供真实 DAG 变化正对照, V_mcar/mar/mnar/V_length/V_obs,* 供缺失/样本量/观测噪声混淆生成器); 其发现"PCMCI+ 在消融中会输出无向瞬时边"是 idea J 通道潜在伪影的具体警示, 应作为待排除混淆源之一。反引文仅 1 篇 (Causal-Audit, 已入库), 印证该子领域仍在 idea 周边形成中。
- arXiv:2602.19903 (Butler/Machlanski/Dimitrakopoulos/Tsaftaris, 2026-02): 实证 GC/TE/PCMCI/DYNOTEARS/VARLiNGAM 对窗口长度 Q 与降采样因子 k 均高度敏感 (GC 在 Q≈33-35 处发生相变), 呼吁建立"因果 Nyquist 定理"但未形式化。**双变量延迟滤波器合成设置可作 Stage-1 gate 的低成本 positive-control 候选**, 其纯超参数驱动的相变现象也是 J 型突变可能只是伪影而非真实结构变化的具体反例, 补入现有五类混淆解释之外的候选第七类 (窗口长度/采样率错配)。
- arXiv:2310.18212 (Machlanski/Samothrakis/Clarke, CLeaR 2024): 8 个经典 IID 因果结构学习算法对超参数误设的鲁棒性剖面与其 oracle 最优表现几乎正交——"最优的算法未必是安全的算法"。建议 Stage-1.5/2 官方 PCMCI+/LKIF gate 设计中显式固定超参数作为实验控制变量, 避免把超参数漂移误判为时间窗口驱动信号。

### 回应"matched negative controls"设计

- arXiv:2412.10039 (Petersen, UAI 2025, R-Score A/21): 提出用"随机猜测"作为因果发现算法评估的通用负对照基线, 对骨架级指标 (precision/recall/F1 等) 给出超几何分布下的精确解析检验, 对无法解析求解的指标提出四步模拟负对照管线。核心警示"免费午餐"现象——不报密度信息的单一指标数值在随机猜测下也可能很漂亮。**方法论呼应 idea 已采用的 paired-trajectory + calibration null percentile 设计**, 可作动机段落支撑引用; 其超几何精确检验可零成本复用于未来"PCMCI+/VAR 恢复的因果骨架是否优于随机猜测"附加诊断 (目前非目标)。
- arXiv:2210.00528 (DANCE, Kummerfeld/Lim/Shi): 数据驱动识别合法 disconnected negative control 变量三元组 (vanishing tetrad rank test), 取代纯专家知识识别。问题设定 (横截面代理变量校验) 与 idea (时间窗口扰动) 不同, 仅方法论姿态可类比引用。
- arXiv:2507.14528 (PU-learning 控制组构建): 若课题未来进入真实遥感/环境数据缺乏干净非事件对照期的阶段, 其两步 positive-unlabeled 控制组构建流程可直接复用, 但目前非目标 (idea 已有 matched ramp-offset 负对照设计)。

### 回应"PCMCI+/LKIF"具体设计 (含未能读全文但值得记录的发现)

- **arXiv:2502.14719 反引文中发现两篇高优先级但因无 arXiv ID 无法通过 arxiv-tool 读全文的真实世界 PCMCI+ 评测** (已如实记录, 未虚构或强行编 ID):
  - Miersch, Günther, Runge et al. (2025, DOI:10.1175/aies-d-24-0114.1, AIES 期刊): 在 45 个真实流域上评测 PCMCI+ 鲁棒性, 发现即使样本量很大, 相当比例的推断因果边仍不一致, 部分关键水文机制被系统性遗漏。
  - Gamella, Peters, Bühlmann (2025, DOI:10.1038/s42256-024-00964-x, Nature Machine Intelligence): Causal Chambers 真实物理测试床上多个因果发现任务案例研究之一报告 PCMCI+ 表现接近随机猜测。**与 idea 自己 Stage-1 pilot 的 NEGATIVE 结果构成直接的警示性平行案例**——建议下一版 refinement 引用作为"PCMCI+ 在真实数据上普遍脆弱, 不仅限于本 testbed"的独立佐证, 若后续需要全文精读, 需通过其他非 arxiv-tool 合规渠道获取 (当前工具链无法处理无 arXiv ID 文献)。
- 全部 18 篇中, 与 idea 当前"因果 vs 非因果模型脆弱性可分性"这一未合上的最大缺口 (v2 review Missing evidence signals) 直接相关的新发现: 无 (本轮未找到同复杂度非因果模型脆弱性基线的新候选, 已有候选 TRACE arXiv:2601.21135 仍是目前最佳选项, 见 topic-scope iteration 2 记录)。

### 本轮新增论文清单 (供上层 topic-scope 并入 landscape, 本 idea-scope 不直接写 landscape)

| arxiv_id | 标题 | 年份 | 一句话解读 | 相关设计点 |
|---|---|---|---|---|
| 2210.00528 | DANCE: 数据驱动负对照变量验证 | 2022 | vanishing tetrad test 验证 disconnected negative control | matched negative controls (方法论类比) |
| 2410.19412 | RCV-PCMCI/VarLiNGAM: k 折鲁棒化因果发现 | 2024 | Consistency/Variability 与 (D,J) 数学同源但用途相反 | D/J profile (本轮最强撞击) |
| 2507.14528 | PU-learning 观测因果推断控制组构建 | 2025 | 无对照组时用 positive-unlabeled 学习挑高置信度对照 | matched negative controls |
| 2510.03959 | 雷暴驱动停电两阶段早期预警 | 2025 | "causal"=无泄漏特征工程, 非结构因果; 应用域相关不撞车 | PCMCI+/LKIF (应用背景) |
| 2508.01848 | TD2C: 监督式因果发现 (互信息特征化) | 2025 | 主动消除假设违背求更准图, 与 idea 方向相反 | PCMCI+/LKIF (镜像对照) |
| 2606.23880 | GRACE: L0 门控精炼因果骨架 | 2026 | bootstrap 多数投票丢弃窗口不稳定性 | D/J profile (镜像对照) |
| 2402.10300 | 高维分岔嵌入早期预警信号 | 2024 | 神经网络 EWS, 经典慢变 regime, 非因果 | 背景/motivation 类比 |
| 2509.01601 | 无序辅助早期预警 (凝聚态物理) | 2025 | 无序拉长预警窗口的物理直觉类比 | 背景/motivation 类比 |
| 2605.03045 | TCD-Arena: 33 类假设违背鲁棒性基准 | 2026 | 现成正对照 regime + 混淆生成器库, R-Score S/22 | synthetic stop gate (高价值) |
| 2602.21381 | VCDF: 一致性/变异性验证层 | 2026 | 同 2410.19412 作者后续版, 承认未验证真实结构变化情形 | D/J profile (本轮最强撞击) |
| 2602.19903 | 因果 Nyquist: 窗口长度/采样率敏感性 | 2026 | GC/PCMCI/DYNOTEARS 等对 (Q,k) 高度敏感的实证 | synthetic stop gate |
| 2310.18212 | 超参数误设对因果结构学习鲁棒性 | 2023 | 鲁棒性剖面与 oracle 最优表现近乎正交 | synthetic stop gate (HP 控制) |
| 2402.09305 | Causal Pretraining: 因果发现基础模型 | 2024 | 监督端到端映射到因果图, 方向正交 | PCMCI+/LKIF (镜像对照) |
| 2303.01412 | 超参数/指标选择对 CATE 估计的影响 | 2023 | 横截面 CATE 而非时序因果发现, 方法论警示 | (背景, 优先级低) |
| 2402.04952 | 基于分离陈述的因果图距离族 | 2024 | Tigramite 兼容, 可严格化 J 通道定义 | D/J profile |
| 2510.24639 | DOTS: 扩散排序时序因果发现 | 2025 | 多噪声尺度排序软投票求更准结构, 方向正交 | PCMCI+/LKIF (镜像对照) |
| 2502.14719 | PC 系方法自洽性 coherency score | 2025 | PCMCI+ 团队本人提出, 零额外成本第三诊断通道候选 | D/J profile + PCMCI+/LKIF (高价值) |
| 2412.10039 | 随机猜测负对照评估因果发现算法 | 2024 | 超几何精确检验 + 模拟负对照管线 | matched negative controls |
